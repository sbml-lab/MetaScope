...
namespace VugMap.Utility.Data
{
	...
	public class DataType
	{
		...
		public		static int						N_INDEXSIZE						= ( 15 * 1000 * 1000 ) / N_INDEXSPAN;		// 15Mbp (bacteria 최대 게놈 커버)
		public		static int						N_KEYRANGE						= 300 * 1000 * 1000;						// BigInteger 키 계산용 (변경 금지)
		...
		private		Brush							m_bshFeature					= null;
		...
		private		DataFeature[]					m_arrFeature					= null;				// read-only: 정렬된 배열
		...
		public void DoAdjust( double dMultiply, double dShift, int nWidth, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{
		...
		public void DoAssignId( string strIdPattern, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{
			if( m_bReadOnly )		return;
		...
		public LinkedListNode< DataFeature > GetFeatureLinkFirst()
		{
			...
			// read-only: 배열에서 단독 노드 반환 (.Value 접근 가능, 순회는 GetFeatureArray() 사용)
		...
		public Brush DoBrushGet()
		{
			return m_bshFeature;
		}

		public void DoBrushSet( Brush bsh )
		{
			m_bshFeature	= bsh;
		}
		...
		{
			SolidColorBrush	scb				= m_bshFeature as SolidColorBrush;
			string			strColor		= string.Format( "{0:X2}{1:X2}{2:X2}", scb.Color.R, scb.Color.G, scb.Color.B );

			return strColor;
		}

		public void DoSearch( string strSearch, bool bCase, ListFeature lstReturn, DialogSearch.DelegateDoSearchProgressUpdate del )
		{
		...
		// read-only: End >= nPos 인 첫 인덱스 반환 (Start 기준 정렬이므로 선형 탐색)
		...
		// read-only 모드: binary search로 Start >= nPosStart 인 첫 인덱스 반환
			...
			public ListDataFeatureNode GetFeatureLinkByStart( int nPosStart )
		{
			if( m_bReadOnly )
			{
				return null;	// read-only: caller는 GetFeatureIndexByStart() 사용
			}
			...
			public ListDataFeatureNode GetFeatureLinkByEnd( int nPos )
		{
			if( m_bReadOnly )
			{
				return null;	// read-only: caller는 GetFeatureIndexByEnd() 사용
			}
		...
		public void BuildIndex()
		{
			...
			// read-only 모드: 배열 구축 후 LinkedList 해제
			if( m_bReadOnly )
			{
