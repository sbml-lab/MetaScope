...
namespace VugMap.Utility.Command
{
	...
	public class CommandEdit : CommandBase
	{
		...
		{
			//	Remove old "current" from Add list, add new one
			ListFeature		lstAdd			= GetFeatureListAdd( m_pnlOwner.DataTypeSelected );
			lstAdd.Remove( dfOld );
			lstAdd.Add( dfNew );

			//	Remove list stays the same (original feature)
			m_dfCurrent		= dfNew;
		}
