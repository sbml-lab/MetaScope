...
namespace VugMap.Utility.Data
{
	...
	public class DataFile
	{
		...
		public void AddDataTypeByIntegrationPorf( DataType dtStart, DataType dtStop, DataType dtProteome, DialogIntegrationOperation.DelegateDoProgressSet delProgress, DialogIntegrationOperation dlg )
		{
			...
			if( strStrand == "+" )
			{
				...
				while( lStart != null )
				{
					...
					{
						dlg.Dispatcher.BeginInvoke( delProgress, DispatcherPriority.Normal, nCount, nTotal );					
						sw.Restart();
					}
					else
					{
		...
		public void AddDataTypeByIntegrationRts( DataType dtRbr, DataType dtTd, DialogIntegrationOperation.DelegateDoProgressSet delProgress, DialogIntegrationOperation dlg )
		{
		...
		public void AddDataTypeByIntegrationTu( DataType dtTss, DataType dtRts, DataType dtPorf, DialogIntegrationOperation.DelegateDoProgressSet delProgress, DialogIntegrationOperation dlg )
		{			
			...
			if( delProgress != null )
			{			
				dlg.Dispatcher.BeginInvoke( delProgress, DispatcherPriority.Normal, 100, 100 );					
			}
		...
		public void AddDataTypeByDiff( List< PnlMapLane > lst, PnlMapLane pnlDiff, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{
		...
		public void AddDataTypeByFilter( List< PnlMapLane > lst, PnlMapLane pnlFilter, EFilterMethod eMethod, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{
		...
		public void AddDataTypeByCopyFeature( PnlMapLane pnlFrom, PnlMapLane pnlTo, DialogFeatureOperation.DelegateDoProgressSet delProgress, DialogFeatureOperation dlg )
		{			
			int				nCount			= 1;
		...
		public void AddDataTypeByMoveFeature( PnlMapLane pnlFrom, PnlMapLane pnlTo, DialogFeatureOperation.DelegateDoProgressSet delProgress, DialogFeatureOperation dlg )
		{			
			int				nCount			= 1;
			int				nTotal			= pnlFrom.ListFeatureSelected.Count;
			string			strType			= pnlTo.DataTypeSelected.Type;
		...
		public void AddDataTypeByMergeFeature( List< PnlMapLane > lst, EMergeMethod eMethod, DialogFeatureOperation.DelegateDoProgressSet delProgress, DialogFeatureOperation dlg )
		{	
			StringBuilder		sbType			= new StringBuilder();
			int					nTotal			= 0;
		...
		public void AddDataTypeByMerge( List< PnlMapLane > lst, EMergeMethod eMethod, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{	
			StringBuilder		sbType			= new StringBuilder();			
			int					nTotal			= 0;
			...
			/*
			StreamWriter	stw				= new StreamWriter( "merge.csv" );
			foreach( KeyValuePair< BigInteger, DataFeature > kv in dicFeature )
			{
				stw.WriteLine( string.Format( "{0},{1}", kv.Value.Start, kv.Value.End ) );
			}
			stw.Close();
			*/
		...
		public void AddDataTypeByAverage( List< PnlMapLane > lst, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg, bool bByMedian )
		{			
		...
		public void AddDataTypeBySum( List< PnlMapLane > lst, DialogLaneOperation.DelegateDoProgressSet delProgress, DialogLaneOperation dlg )
		{			
			LinkedListNode< DataFeature >[]	lA				= new LinkedListNode< DataFeature >[ lst.Count ];
			StringBuilder					sbType			= new StringBuilder();
		...
		public void DoSave( string strFile )
		{
			...
			{
				Logger.Logger.PrintLine( "# ERROR, DataFile:DoSave - {0}", e.ToString() );
				Error.ErrorMessage.ShowError( string.Format( "Failed to save file \"{0}\".\r\n\r\n{1}", strFile, e.Message ) );
			}
			finally
			{
				if( sw != null )		sw.Close();
		...
		public DataType GetDataType( string strSequenceId, string strType )
		{
			...
			if( m_dicDataType.Keys.Contains( strKey ) == false )
			{
				...
				{
					Brush	bsh				= m_dicDataType.Values.First().DoBrushGet();
					dt.DoBrushSet( bsh );
				}
