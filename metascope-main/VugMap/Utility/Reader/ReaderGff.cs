...
namespace VugMap.Utility.Reader
{
	public class ReaderGff
	{
		...
		private		Stream							m_stmExternal					= null;		// 외부에서 전달된 스트림 (gz/zip)
		...
		public bool DoReadFile()
		{
			...
			try
			{
				...
				srFile			= new StreamReader( stm, System.Text.Encoding.Default );
				...
				while( srFile.Peek() > -1 )
				{
					...
					{
						Logger.Logger.PrintLine( "# WARNING, ReaderGff:DoReadFile - skipped line {0} (parse error)", nCountLine );
						nCountSkipped++;
						nCountLine++;
						continue;
					}
				...
				//Logger.Logger.PrintLine( string.Format( "# ReaderGff:DoReadFile() {0} lines read", nCountLine ) );
			}
			catch( Exception e )
			{
				string			strException	= e.Message;
				string			strLogException	= string.Format( "# ERROR, ReaderGff:DoReadFile - {0}", strException );
				Logger.Logger.PrintLine( strLogException );
			...
			Logger.Logger.PrintLine( strLog );

			if( nCountSkipped > 0 )
			{
				Logger.Logger.PrintLine( "# WARNING, ReaderGff:DoReadFile - {0} lines skipped due to parse errors", nCountSkipped );
			}
